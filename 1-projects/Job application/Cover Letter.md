...
For a transport modelling project for Gatwick Airport we had to develop a large model at the edge of what the software typically handles. (24 hours model with over 90 origin destinations as opposed to usual 1 hour and 15 OD) This meants that the standard VBA COM interface, which allows to automate the data inputs, slowed to a crawl. The import process took around 3 hours. I bypassed the standard interface and rewrote the required features with a set of python scripts, cutting the import process down to around 10 minutes. The hack 


Faced with a massive 24-hour model bottlenecked by slow data imports, I bypassed the software's standard VBA interface and leveraged its less-documented Python API. This 'hack' slashed import times from over three hours to under five minutes. By optimizing the process—disabling UI updates and refining import order—we gained crucial time for calibration, significantly enhancing model quality. Beyond that, I built custom scripts to overcome software limitations: one simulated vehicle recirculation in parking areas, revealing critical congestion issues, and another automated the transfer of demand data between different model scales, eliminating manual errors and saving substantial time. Essentially, I found where the system was slow or lacking, and rewrote parts of it to do what we needed.