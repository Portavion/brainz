...
For a transport modelling project for Gatwick Airport we had to develop a large model at the edge of what the software typically handles. (24 hours model with over 90 origin destinations as opposed to usual 1 hour and 15 OD) This meant that the standard VBA COM interface, which allows to automate the data inputs, slowed down to a crawl. The import process took around 3 hours. I bypassed the standard interface and rewrote the required features with a set of python scripts, cutting the import process down to around 10 minutes. The hack saved time by bypassing the software UI updates and refined the import steps order to minimise the amount of background db queries. This allowed us to do more calibration iteration and deliver a much higher quality model.

For a transport modeling project at Gatwick Airport, I had to develop a significantly larger model than the software was typically designed for(a 24-hour model with over 90 origin-destination pairs, compared to the usual 1-hour, 15-pair models) This caused the standard VBA COM interface, used for automating data inputs, to slow down to a crawl, with the import process taking upward of three hours. To solve this, I bypassed the standard interface and rewrote the necessary features with a set of Python scripts. I managed to reduce the import time to around 10 minutes and the scripts are now standard for all modelling projects. This optimisation was done by disabling software UI updates and refining the import step order to minimize background database queries. As a result, we were able to perform more calibration iterations, leading to a much higher quality model.